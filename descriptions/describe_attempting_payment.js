...
*/
const describeAttemptingPayment = (args, cbk) => {
  return new Promise((resolve, reject) => {
    asyncAuto({
      // Check arguments
      validate: cbk => {
        if (!args.db) {
          return cbk([400, 'ExpectedDbToDescribeAttemptingPayment']);
        }

        if (!args.mtokens) {
          return cbk([400, 'ExpectedMtokensToDescribeAttemptingPayment']);
        }

        if (!args.out_channel) {
          return cbk([400, 'ExpectedOutChannelToDescribeAttemptingPayment']);
        }

        if (!args.public_key) {
          return cbk([400, 'ExpectedPublicKeyToDescribeAttemptingPayment']);
        }
...
export { describeAttemptingPayment }
