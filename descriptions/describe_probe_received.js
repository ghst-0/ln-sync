...
*/
const describeProbeReceived = (args, cbk) => {
  return new Promise((resolve, reject) => {
    asyncAuto({
      // Check arguments
      validate: cbk => {
        if (!args.db) {
          return cbk([400, 'ExpectedDbToDescribeProbeReceived']);
        }

        if (!args.in_channel) {
          return cbk([400, 'ExpectedInChannelToDescribeProbeReceived']);
        }

        if (!args.public_key) {
          return cbk([400, 'ExpectedPublicKeyToDescribeProbeReceived']);
        }
